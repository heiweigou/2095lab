...
storageEngine = function() {
	...
	return {
		...
		initObjectStore  : function(type, successCallback, errorCallback) {	//success callback is function() {} i.e. the empty function //UPDATE successCallback is function() {callback()} to accommodate the asynchronism of IndexedDB
	    	...
	    	if (exists) {
	    		//following callback contains loadTasks which can now be executed because db open and tasks object store exists
	    		successCallback(null);	//success callback is function() {} i.e. the empty function, only squeal if there is a failure (see else clause) //UPDATE successCallback is function() {callback()} to accommodate the asynchronicity of IndexedDB
	    	} 
				...
				request.onsuccess = function(event) {
                    //following callback contains loadTasks which can now be executed because db open and tasks object store exists
					successCallback(null);	//success callback is function() {} i.e. the empty function //UPDATE successCallback is function() {callback()} to accommodate the asynchronicity of IndexedDB
				};
				request.onerror = function(event) {
	    ...
	    save : function(type, obj, successCallback, errorCallback) { 
			...
			//clean up the id
	    	if (!obj.id)	//toObject will return id="" (= falsy) for new, unsaved tasks (the id field is hidden and will be empty), we delete that property, db auto-increment will create one
	    		delete obj.id ;
	    	...
	    	var request = objectStore.put(obj);		//ASYNC NO WAIT indexedDB knows about objects and how to store them in a specified data store, "put" operation adds (new key) or updates (existing key), key was defined as id property when object store was created (see above)
		...
		saveAll:function (type, objArray, successCall, errorCall) {
			if (!database){
				errorCall('storage not initialised','engine not initialised')
			}
			var tx=database.transaction([type]);
			tx.oncomplete=function (event) {
				errorCall('transaction_error','not possible')
            }
			...
			$.each(objArray,function (i, obj) {
				if(!obj.id)
					...
					obj.id=parseInt(pbj.id);
				var request=objectStore.put(obj);
				request.onsuccess=function (event) {

                }
                request.onerror=function (event) {
					errorCall('object not stored','not possible')
                }
            })
